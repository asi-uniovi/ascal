...
class HVReactiveAutoscaler(Autoscaler):
    ...
    def run(self, app_workloads: dict[App, RequestsPerTime]) -> AutoscalerStatistics:
        ...
        if self.time % self._time_period == 0:
            ...
            out_of_tolerance = \
                self._check_out_of_tolerance_cpu_utils(self._desired_cpu_utilization, self._tolerance_util)
            # If any transition is completed and utilizations are out of tolerance
            if self._timedops.is_event_list_empty() and out_of_tolerance:
                # Calculate a new allocation and the corresponding transition
                self.allocation = self._new_allocation
