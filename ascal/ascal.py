...
class Ascal:
    ...
    def __init__(self, ascal_config: AscalConfig, log=None, metrics_period:int = 60):
        """
        Ascal constructor. It assumes overcommittment of CPU cores, i.e., application's containers can 
        receive more cores those committed when other applications are under low workload conditions.
        ...
        # Samples of node CPU utilization. Samples are averaged in the metrics period
        self._cpu_util_samples: dict[Vm, list[float]] = {} 

        # Average CPU utilization in each node
        self.avg_cpu_util: dict[Vm, float] = {} 

        # List of application performances, one per second
        self.app_perf: dict[App, list[float]] =  {app: [] for app in self._autoscaler.apps}
    ...
    def _get_perf_per_core(self) -> dict[App, float]:
        ...
        for app_fm in self._autoscaler.system:
            cores = self._autoscaler.system[app_fm].cores.to("cores").magnitude
            perf = self._autoscaler.system[app_fm].perf.to("req/s").magnitude
            a, _ = app_fm
            perf_per_core[a] = perf / cores
        return perf_per_core

    @staticmethod
    def _get_app_node_committed_cores(nodes: list[Vm]) -> dict[App, dict[Vm, float]]:
        """
        Get application committed cores in each node. 
        It assumes that containers being removed (associated to None applications) are ignored.
        :param nodes: List of nodes allocating at least one container that is not being removed (cc.app != None).
        :return: Application's committed CPU cores in each node. 
        It is a dictionary with applications as keys and dictionaries as values. The inner dictionaries have nodes
        as keys and committed cores in cores as values."""
        ccai = {}
        for node in nodes:
             for cg in node.cgs:
                app = cg.cc.app
                ...
                if node not in ccai[app]:
                    ccai[app][node] = 0.0
                ccai[app][node] += cg.cc.cores.to("cores").magnitude * cg.replicas
        return ccai

    def _get_app_cores(self, workload: dict[App, RequestsPerTime], allocation: Allocation,
                       va: dict[App, float], overcommit: bool=True) -> dict[App, dict[Vm, float]]:
        """
        Get application cores in each node for the current workload. It assumes weighted-round-robin 
        load balancing and container allocation cores with committment or 
        overcommitment (similar to requests in Kubernetes).
        ...
        :param overcommit: If it is True, it assumes overcommitment of container cores. 
        Otherwise, it assumes committment. 
        ...
        wa = {
            app: (workload[app].to("req/s").magnitude + self._pending_workload[app]) 
            for app in workload
        }
        ...
        # Application cores to calculate
        cai = {app: {} for app in workload}

        # Get committed cores for each application and node pair
        ccai = Ascal._get_app_node_committed_cores(nodes)

        # Total committed cores for applications Aa
        ...
        # Total committed cores for applications allocated in nodes Ni
        ...
        ci = {node: node.ic.cores.to("cores").magnitude for node in nodes}
            ...
            while True:
                ...
                for node in dict(mi):
                    if len(node_apps[node]) > 0:
                        # Update the available cores multiplier in the node with overcommitment of cores
                        if overcommit:
    ...
    def _get_avg_cpu_util(self, cpu_util_sample: dict[Vm, float]) -> dict[Vm, float]:
        """
        Update CPU utilization samples in each node and get the average CPU utilization in each node.
        :param cpu_util_sample: New CPU utilization sample for each node.
        :return: Average CPU utilization in each node.
        """
        current_nodes = list(cpu_util_sample.keys())
        for node in list(self._cpu_util_samples):
            if node not in current_nodes: # Nodes may change
                del self._cpu_util_samples[node]
        for current_node in current_nodes:
            if current_node not in self._cpu_util_samples:
                self._cpu_util_samples[current_node] = []
        # Remove the oldest subsample when the number of subsamples is equal to the metrics period
        if len(self._cpu_util_samples) == self._metrics_period:
            self._cpu_util_samples.pop(0)
        for node in current_nodes:
            assert round(cpu_util_sample[node], 6) <= 1.0 # Check the validity of CPU utilizations
            self._cpu_util_samples[node].append(cpu_util_sample[node])

        return {
            key: sum(self._cpu_util_samples[key]) / len(self._cpu_util_samples[key])
            for key in self._cpu_util_samples
        }
    ...
    def run(self, break_point: int | None = None):
        ...
        while self.time < break_point:
            ...
            for app in self._autoscaler.apps:
                workload = RequestsPerTime(f"{self._workload_vectors[app][self.time] * 3600.0}  req/hour")
            ...
            perf = {
                app: app_perf_per_core[app] * sum(cai[app][node] for node in cai[app]) 
                for app in self._autoscaler.apps
            }
            for app in self._autoscaler.apps:
                self.app_perf[app].append(perf[app])
            ...
            for app in self._autoscaler.apps:
                if round(self._pending_workload[app], 6) > 0.0:
                    added_qwts[app] = round(self._pending_workload[app])
            if len(added_qwts) > 0:
    ...
    def get_performances(self, committed=True) -> dict[str, list[float]]:
        """
        Get application performances.
        :param committed: If it is True the returned performances are committed performances. Committed 
        application performances are maximum values that come from committed cores. Otherwise, overcommittment
        of CPU cores is assumed, so application performances can be higher than committed values when other 
        applications experience low workloads.
.       :return: For each application the performances in req/s at every second.
        """
        if committed:
            ...
            previous_time = -1
            for current_time, current_nodes in self.allocation_changes:
            ...
            app_perfs = {str(app): self.app_perf[app] for app in self.app_perf}
        return app_perfs
    
    def get_cluster_cost(self) -> list[float]:
        """
        Get the cluster cost in $/hour.
    ...
    def get_queue_waiting_times(self, committed=True) -> dict[str, list[float]]:
        """
        ...
        Each application is modeled as a D/D/m queue with heterogeneous servers:
        ...
        :param committed: It writes queue waiting times under committed performances when it is True 
        and queue times under overcommitted CPU cores otherwise.
        :return: One-second samples of queue waiting times.
        """
        if committed:
            # Queue waiting times are obtained using the committed application performances
            app_perfs = self.get_performances(committed=committed)
            ...
            frac_surplus = {app_name: 0.0 for app_name in app_perfs}
            queue_length = {app_name: [0] for app_name in app_perfs}
            for app in self._workload_vectors:
                app_name = str(app)
                for w, p in zip(self._workload_vectors[app][1:], app_perfs[app_name][1:]):
                    w_frac = w - int(w)
            ...
            waiting_times = {
                app_name: [ql / wperf for ql, wperf in zip(queue_length[app_name], app_perfs[app_name])]
                for app_name in queue_length
            }
        else:
            # Queue waiting times with overcommitted CPU cores
            waiting_times = {str(app): [] for app in self._autoscaler.apps}
            previous_time = {app: -1 for app in self._autoscaler.apps}
            for current_time, qwts_sample in self.queue_waiting_times:
                for app, qwt_val in qwts_sample.items():
                    # Fill with zeroes when there is a gap between the current and previous times
                    if current_time - previous_time[app] > 1:
                        waiting_times[str(app)].extend([0.0] * (current_time - previous_time[app] - 1))
                    # Append the current queue waiting times
                    waiting_times[str(app)].append(qwt_val)
                    previous_time[app] = current_time
            for app in self._autoscaler.apps:
                # If the last samples are zeroes
                n_samples = len(waiting_times[str(app)]) 
                if n_samples < self.time + 1:
                    waiting_times[str(app)].extend([0.0] * (self.time + 1 - n_samples))
        return waiting_times

    def write_workload_csv(self, csv_file: str) -> None:
        """
        Write a csv file with the workload in req/s for every application and time.
    ...
    def write_performance_csv(self, csv_file: str, committed=True) -> None:
        """
        Write a csv file with the performance for every application and time.
        :param csv_file: csv file to write.
        :param committed: It writes committed performances when it is True and performances under
        overcommittment otherwise.
    ...
    def write_allocations(self, yaml_file: str):
        ...
        with open(yaml_file, "w") as f:
            for current_time, alloc in self.allocation_changes:
